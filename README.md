This repository provides MATLAB code for the Least-Squares Spatio-Temporal Adjustment Algorithm (LSSTAA) that conducts relative radiometric normalization of satellite image time series (2025, ISPRS). It was demonstrated on PlanetScope Dove-Classic and Dove-R time series.

LSSTAA constructs a network graph connecting overlapping images (shown below), where each link represents their radiometric relationship. The optimal solution for the entire network is then obtained through a least-squares adjustment, a well-established technique in surveying and photogrammetry.
...
The figure below shows example mosaics before and after normalization. The LSSTAA normalization reduces seamlines, and inter-image brightness and color differences in the mosaic, producing radiometrically consistent time series.

<p align="center">
  <img src="images/doy174_before.png" width="45%">
  <img src="images/doy174_after.png" width="45%">
</p>
...
3. **Run the script**  
   ...
   The results (normalized time series stack with a .hdr file, per-band normalization coefficients, and three example mosaics) will be saved automatically in a new folder named **`output`** within the data directory.


### Data

Two datasets are provided in the `data/` folder:
- **data1.zip** — the original PlanetScope time series used for testing.  
  It contains the last ten images of the Site I dataset described in the paper, acquired over Bié Province, Angola.  
...
- **data1_results.zip** — example output of per-band normalization coefficients generated by LSSTAA using test data 1 at the developer’s end for reference.  
