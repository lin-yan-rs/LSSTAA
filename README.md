...
LSSTAA constructs a network graph connecting overlapping images (shown below), where each connection represents their radiometric relationship. The optimal solution for the entire network is then obtained through a least-squares adjustment, a well-established technique in surveying and photogrammetry.
...
- **data1.zip** — PlanetScope time series used for testing.  
  It contains the Site I dataset acquired over Bié Province, Angola as described in the paper. Only the last 10 images in the dataset (15 images) are included to allow effective visualization of pairwise scatterplots (using the full series would make scatterplots too dense and time-consuming to render). The images were resampled by a factor of 10 for faster computation.
- **data1_results.zip** — per-band normalization coefficients generated by LSSTAA using test data 1, provided as a reference for results obtained at the developer’s end.
